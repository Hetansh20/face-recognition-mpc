...
interface AttendanceApiService {
    ...
    suspend fun recognizeFrame(@Path("id") sessionId: String, @Part image: MultipartBody.Part): ApiResponse<FrameRecognitionResult>
    @Multipart
    @POST("attendance/sessions/{id}/capture")
    suspend fun captureGroupPhoto(@Path("id") sessionId: String, @Part image: MultipartBody.Part): ApiResponse<FrameRecognitionResult>
    @Multipart
    @POST("attendance/sessions/{id}/review")
    suspend fun reviewGroupPhotos(@Path("id") sessionId: String, @Part images: List<MultipartBody.Part>): ApiResponse<GroupReviewResult>
    @POST("attendance/sessions/{id}/confirm")
    suspend fun confirmAttendance(@Path("id") sessionId: String, @Body request: ConfirmAttendanceRequest): ApiResponse<ConfirmAttendanceResult>
    @POST("attendance/sessions/{id}/stop")
    suspend fun stopSession(@Path("id") sessionId: String, @Body request: StopSessionRequest): ApiResponse<SessionSummary>
    ...
    suspend fun updateSessionRecords(@Path("id") sessionId: String, @Body request: UpdateRecordsRequest): ApiResponse<SessionResultsData>
    @GET("reports/export/session-csv")
    suspend fun exportSessionPresentCsv(@Query("session_id") sessionId: String): okhttp3.ResponseBody
    @GET("reports/export/session-absent-csv")
    suspend fun exportSessionAbsentCsv(@Query("session_id") sessionId: String): okhttp3.ResponseBody
}
...
class AuthInterceptor(private val prefs: SharedPreferences) : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val builder = original.newBuilder()
            ...
            .header("X-App-Version", "v${BuildConfig.VERSION_NAME}")
...
object ApiClient {
    ...
    fun setServerAddress(address: String, context: Context) {
        ...
        if (!clean.contains("/api/v1")) clean = if (clean.endsWith("/")) "${clean}api/v1/" else "$clean/api/v1/"
